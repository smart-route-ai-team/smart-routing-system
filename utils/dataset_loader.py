...
class ProtocolDataset:
    ...
    def annotate_graph(self, G: nx.Graph) -> nx.Graph:
        ...
        for u, v, data in G.edges(data=True):
            proto   = protocols[(u + v) % len(protocols)]
            metrics = self.sample_edge_metrics(protocol=proto, u=u, v=v)
            data.update(metrics)
        return G
    ...
    def refresh_edge_traffic(self, G: nx.Graph) -> nx.Graph:
        for u, v, data in G.edges(data=True):
            ...
            metrics = self.sample_edge_metrics(protocol=proto, u=u, v=v)
            metrics["protocol"]         = data.get("protocol", metrics["protocol"])
            metrics["protocol_fitness"] = data.get("protocol_fitness", metrics["protocol_fitness"])
            data.update(metrics)
        return G
